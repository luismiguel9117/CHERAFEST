  ...
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.play().catch(err => {
        console.log("Autoplay con audio bloqueado por el navegador:", err);
      });
    }
  }, []);
    ...
    <div className="fixed inset-0 z-[200] bg-[#2b1520] flex flex-col items-center justify-center animate-fadeIn overflow-hidden">
      ...
      <div className="absolute top-6 left-0 right-0 flex justify-center items-center pointer-events-none px-4">
        <div className="bg-white/90 backdrop-blur-md border-2 border-[#ffd0e2] rounded-full px-6 py-2 shadow-lg flex items-center gap-2">
          ...
          <span className="font-baloo font-bold text-lg md:text-xl text-[#b7407a] tracking-wide">
            BIENVENIDOS AL CHARO FEST 2026
          </span>
          <Sparkles className="w-5 h-5 text-[#bda3e8] animate-spin" />
        </div>
      </div>

      {/* Botones de acción inferiores */}
      <div className="absolute bottom-10 left-0 right-0 flex flex-col sm:flex-row justify-center items-center gap-4 px-6 z-10">
        <button
          onClick={toggleSound}
          className="font-baloo font-bold text-base md:text-lg text-[#b7407a] bg-white/95 hover:bg-white border-2 border-[#ffc0d8] rounded-full px-6 py-3 shadow-[0_6px_0_rgba(255,160,200,0.55)] hover:translate-y-[-2px] transition-all flex items-center gap-2"
        >
          {isMuted ? (
            <>
        ...
        <button
          onClick={handleEnter}
          className="font-baloo font-extrabold text-lg md:text-xl text-white bg-[#ef7fae] hover:bg-[#e0669a] border-2 border-white rounded-full px-8 py-3.5 shadow-[0_6px_0_rgba(190,70,125,0.5)] hover:scale-105 active:scale-95 transition-all flex items-center gap-3 group"
        >
